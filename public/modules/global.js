...
class global {
  constructor(id, opt) { if (opt.type == "new") this.raw = document.createElement(id);
    ...
    this.keep = {}; if (opt.type != "ref") { this.raw.id = "ID_" + Math.floor(Math.random() * 99999999999);}
      this.id = this.raw.id ; return this }
  ...
  text(dat) { if (typeof dat!="undefined") { this.raw.innerText = dat; return this } else return this.raw.innerText }
...
/* Misc */
/* storage base setup */
store = window.localStorage; //if(!store.pstheme)store.pstheme="light";if(!store.psaccent)store.psaccent="#e91e63";if(!store.psback)store.psback="#fcfcfc";if(!store.pstext)store.pstext="#454545";if(!store.psshadow)store.psshadow="#00000055";if(!store.psshade)store.psshade="#55454545";
/* cancel current dialog */
window.addEventListener("popstate", function() { var mod = document.querySelectorAll(".play_modal"); var mod2 = new ref(mod[mod.length - 1]); if (mod2.attr("cancel") == "true") { if (mod.length == 1) body.css({ "overflow": "auto" });
    mod2.animate({ opacity: 0 }, 300, null, function() { mod2.remove() }); } else history.pushState(null, null, null); })